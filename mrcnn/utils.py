...
class Dataset(object):
    ...
    def __init__(self, class_map=None):
        ...
        print('class_info: {}'.format(self.class_info))
...
def resize_image(image, min_dim=None, max_dim=None, min_scale=None, mode="square", bbox=[]):
    ...
    if mode == "square":
        ...
        if np.any(bbox):
            ...
            for i in range(len(scaled_bbox)):
                for j in range(4):
                    #print('bbox[{},{}]={}'.format(i,j,bbox[i,j]))
                    if j==0 or j==2: #y
                    ...
                    #print('scaled_bbox[{},{}]={}'.format(i,j,scaled_bbox[i,j]))
            scaled_bbox=np.rint(scaled_bbox).astype(np.int32)
            return image.astype(image_dtype), window, scale, padding, crop, scaled_bbox
        else:
...
def resize_image_old(image, min_dim=None, max_dim=None, min_scale=None, mode="square"):
    """Resizes an image keeping the aspect ratio unchanged.
...
#def resize_bbox(bbox, window, image, padding):
...
def compute_matches(gt_boxes, gt_class_ids,
                    pred_boxes, pred_class_ids, pred_scores, config,
                    iou_threshold=0.5, score_threshold=0.0, gt_masks=None, pred_masks=None):
    ...
    if config.GENERATE_MASKS:
        ...
        #pred_boxes = pred_boxes[..., indices] #chng
...
def compute_ap(gt_boxes, gt_class_ids,
               pred_boxes, pred_class_ids, pred_scores, config,
               iou_threshold=0.5, gt_masks=None, pred_masks=None):
    ...
    gt_match, pred_match, overlaps = compute_matches(
        ...
        pred_boxes, pred_class_ids, pred_scores, config,
        iou_threshold, gt_masks=gt_masks,  pred_masks=pred_masks)
